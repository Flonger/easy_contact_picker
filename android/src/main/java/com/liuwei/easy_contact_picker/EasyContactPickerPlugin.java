...
public class EasyContactPickerPlugin implements MethodCallHandler, PluginRegistry.ActivityResultListener {
  ...
  private void getContacts(){
    ...
    //获取联系人。按首字母排序
    Cursor cursor = mActivity.getContentResolver().query(uri, null,null,null, ContactsContract.CommonDataKinds.Phone.SORT_KEY_PRIMARY);
    if (cursor != null) {
      ...
      while (cursor.moveToNext()) {
        ...
        String lastUpdateTime = cursor.getString(cursor.getColumnIndex(ContactsContract.CommonDataKinds.Phone.CONTACT_LAST_UPDATED_TIMESTAMP));
        try {
          lastUpdateTime = timeStamp2Date(Long.valueOf(lastUpdateTime));
        } catch (Exception e) {
   ...
   * @return
   */
  public static String timeStamp2Date(long format) {
    String sd = "loading";
    if (format > 0) {
      SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");//这个是你要转成后的时间的格式
      sd = sdf.format(new Date(format));   // 时间戳转换成时间
    }
    if (sd.equals("1970-01-01")) {
      sd = "loading";
    }
    return sd;
  }
